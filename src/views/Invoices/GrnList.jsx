...
const GrnList = () => {
  ...
  const request = (reset_offset = true) => {
    ...
    axios
      .post(
        new URL("/api/v1/supplier/grn/listgrns", themeConfig.backendUrl),
        query
      )
      .then((res) => {
        if (res.data.error) {
  ...
  const basicColumns = [
    ...
    },
    {
      ...
      width: "150px",
      selector: (row) => (
        <Typography style={{ color: "#f26c13" }}>{row.poNo}</Typography>
      ),
    },
    {
      name: "Asn ID",
...

    {
      ...
      column: "grnCode",
      width: "180px",
      selector: (row) => row.grnCode,
      cell: (row) => {
        if (row.grnStatus === null) {
          return <Typography style={{ color: "red" }}>Null</Typography>;
        } else {
          return (
            <Typography style={{ color: "#f26c13" }}>{row.grnCode}</Typography>
          );
        }
      },
    },
    ...
    },
    {
      ...
      width: "200px",
      selector: (row) => {
        const dateParts = row.postingDate.split("-");
    ...
    },
    {
      ...
      width: "200px",
      selector: (row) => {
        const dateParts = row.documentDate.split("-");
        const formattedDate = ` 📅 ${dateParts[2]}-${dateParts[1]}-${dateParts[0]}`;
        return formattedDate;
      },
    },
