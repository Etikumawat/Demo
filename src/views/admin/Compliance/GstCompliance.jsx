...
const PanCompliance = () => {
      ...
      setQuery((prev) => {
        return {
          ...
          filter: {
            startDate: "",
            endDate: "",
            dateField: isDispatchDate ? "dispatchDate" : "gstTime",
          },
        };
      });
    } else {
  ...
  const request = (reset_offset = true) => {
    ...
    axios
      .post(
        new URL(
          "/api/v1/services/masterIndia/gst/list",
  ...
  const basicColumns = [
    ...
    },
    {
      ...
      cell: (row) => <div>{row.gst ? row.gst : "NA"}</div>,
    },
    {
      name: "Old Date",
      column: "oldTime",
  ...
  const exportAll = (isExportSelectedData = []) => {
    setDownloading(true);
    axios.post(
      new URL("/api/v1/services/masterIndia/gst/excelExport", themeConfig.backendUrl),
      query,
      { responseType: "blob" }
    )
      .then((response) => {
        if (response.headers["content-type"].includes("application/json")) {
          ...
          reader.onload = () => {
            ...
            if (jsonData.error) {
              ...
              downloadFile(response.data, "GSTList.xlsx");
            }
          };
