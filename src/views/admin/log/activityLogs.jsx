...
const ActivityLogs = () => {
  ...
  const request = (reset_offset = true) => {
    ...
    axios
      .post(
        new URL("/api/v1/admin/users/newuseractivity", themeConfig.backendUrl),
        query
      )
      .then((res) => {
        if (res.data.error) {
      ...
      setQuery((prev) => {
        return {
          ...
          filter: {
            startDate: "",
            endDate: "",
            dateField: isDispatchDate ? "dispatchDate" : "timestamp",
          },
        };
      });
    } else {
  ...
  const basicColumns = [
    ...
    },
    {
      ...
      cell: (row) => <div>{row.response.message}</div>,
    },
    {
      name: "User",
      column: "username",
