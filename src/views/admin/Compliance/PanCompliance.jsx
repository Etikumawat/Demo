...
const PanCompliance = () => {
      ...
      setQuery((prev) => {
        return {
          ...
          filter: {
            startDate: "",
            endDate: "",
            dateField: isDispatchDate ? "dispatchDate" : "panTime",
          },
        };
      });
    } else {
  ...
  const request = (reset_offset = true) => {
    ...
    axios
      .post(
        new URL(
          "/api/v1/services/masterIndia/pan/list",
  ...
  const basicColumns = [
    ...
    },
    {
      ...
      cell: (row) => {
        ...
        const formattedDate = row.oldTime
          ? formatDate(row.oldTime.slice(0, 10))
          : "NA";
  ...
  const verify = async () => {
    const data = {
      panNo: selectedRowsForVerify,
    };
    setLoading(true);
    await axios
      .post(
        new URL(
          "v1/services/masterIndia/pan/validateSelected",
          themeConfig.backendUrl
        ),
        data
      )
      .then((res) => {
        ...
        request();
        <RefreshCw size={15} />;
        toast.success("Updated list fetched successfully");
        // setData(res.data.data);
      })
      .catch((error) => {
  ...
  const exportAll = (isExportSelectedData = []) => {
    setDownloading(true);
    axios.post(
      new URL("/api/v1/services/masterIndia/pan/excelExport", themeConfig.backendUrl),
      query,
      { responseType: "blob" }
    )
      .then((response) => {
        if (response.headers["content-type"].includes("application/json")) {
          ...
          reader.onload = () => {
            ...
            if (jsonData.error) {
              ...
              downloadFile(response.data, "PANList.xlsx");
            }
          };
